...
const char* obj_database = "";	// ｩwｸq mesh ｪｺｹwｳ]･ﾘｿ
...
{
	FILE	*scene;
	char	token[100], buf[100], v[5][100];	// v[5] ｪ愠ﾜ､@ｭﾓ polygon ｳﾌｦh･i･Hｦｳ 5ｭﾓ vertex
	float	vec[3];

	size_t	n_vertex, n_texture, n_normal;
	size_t	cur_mtl = 0;				// state variable: ･ﾘｫeｩﾒｨﾏ･ﾎｪｺ material
	...
	while(!feof(scene))
	{
		...
		{
			size_t i;
			for (i=0;i<3;i++)		// face ｹwｳ]ｬｰ 3｡Aｰｲｳ]､@ｭﾓ polygon ｳ｣･uｦｳ 3 ｭﾓ vertex
			{
				fscanf(scene,"%s",v[i]);
				//printf("[%s]",v[i]);
			}
			...
			for (i=0;i<3;i++)		// for each vertex of this face
			{
				...
				str[offset] = '\0';
				n_texture = atoi(str);	// case: xxx//zzz｡Atexture ｳ]ｬｰ 0 (tList ｱq 1 ｶ}ｩl)
				base += (ch == '\0')? offset : offset+1;
				offset = 0;
				...
				str[offset] = '\0';
				n_normal = atoi(str);	// case: xxx/yyy｡Anormal ｳ]ｬｰ 0 (nList ｱq 1 ｶ}ｩl)
...
void mesh::LoadMtl(string tex_file)
{
	...
	//fp_mtl = fopen("model/ground.mtl","r");
	fp_mtl = fopen("model/wall.mtl","r");
	//fp_mtl = fopen("model/wall.mtl","r");
	...
	{
		token[0] = NULL;
		fscanf(fp_mtl,"%s", token);		// ﾅｪ token
		...
		if (!strcmp(token,"newmtl"))
		{
			...
			cur_mat = mTotal++;					// ｱq mList[1] ｶ}ｩl｡AmList[0] ｪﾅ､Uｨﾓｵｹ default material ･ﾎ
...
void mesh::Init(const char* obj_file)
{
	...
	vList.push_back(Vec3(default_value));	// ｦ]ｬｰ *.obj ｪｺ index ｬOｱq 1 ｶ}ｩl
	nList.push_back(Vec3(default_value));	// ｩﾒ･Hｭn･ push ､@ｭﾓ default value ｨ・vList[0],nList[0],tList[0]
	tList.push_back(Vec3(default_value));

	material defaultMtl;
	mList.push_back(defaultMtl);
	// ｩwｸq default meterial: mList[0]
	...
	LoadMesh(string(obj_file));		// ﾅｪ､J .obj ﾀﾉ (･iｳBｲz Material)
}
